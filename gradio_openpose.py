...
model_name = 'control_v11p_sd15_openpose_fp16'
model = create_model(f'./models/{model_name}.yaml').cpu()
#model.load_state_dict(load_state_dict('./models/v1-5-pruned.ckpt', location='cuda'), strict=False)
model.load_state_dict(load_state_dict('../models/ldm/stable-diffusion-v1/model.ckpt', location='cuda'), strict=False)
...
def process(det, input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
    ...
    with torch.no_grad():
        ...
        x_samples = model.decode_first_stage(samples.to(torch.float16))
